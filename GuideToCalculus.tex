...
Now that we've established these two cornerstones of Calculus, we can do a lot from these. To demonstrate, let's find the derivative of $\ln x$ . 
\begin{flalign*}
	\text{Let } y &= \ln x \nn
	 e^y &= x \nn
...
\begin{flalign}
	...
	\Deriv{(f \cdot g)}{x} &= f\Deriv{g}{x} + g  \Deriv{f}{g} \ \text{ for any functions $f(x)$ and $g(x)$. } 
	\intertext{Hence, or otherwise, prove that }
	\Deriv{\left( \frac{f}{g} \right)}{x} &= \frac{g  \, \Deriv{f}{x} - f  \, \Deriv{g}{x}}{g^2} \ \text{ for any functions $f(x)$ and $g(x)$.}
\end{flalign}
\newpage
