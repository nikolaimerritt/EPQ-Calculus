...
\begin{flalign*}
	\text{avg velocity} &= \DelFrac{s}{t} = \frac{\text{new dist} - \text{old dist}}{\text{new time} - \text{old time}} \, . \\ \\
...
\\ \\ Let's refer to our incredibly tiny (but not zero!) change in time as \(\Diff{t}\). And, let's refer to our resultant incredibly tiny change in distance as \(\Diff{s}\).
\\ \\Now, we want to find \(\Deriv{s}{t}\). Using our limit definition, we have:
\begin{flalign*}
...
\\ \\Now for some jargon. Mathematicians would refer to our minuscule difference in \(t\), \(\Diff{t}\), as the \textbf{differential} of \(t\), and similarly, \(\Diff{s}\) is called the differential of \(s\). Our rate of change has a name too: \(\Deriv{s}{t}\) is called the \textbf{derivative} of \(s\). To make it clear that \(\Diff{s}\) is being divided by \(\Diff{t}\) (as opposed to some other differential), we could add ``with respect to \(t\)" at the end.
%
...
\begin{flalign*}
	...
	\Deriv{(x^n)}{x} &\equiv nx^{n-1} \ \ \text{for any } n \, .
...
\\ \\ To arrive at this ``golden rule", let's quickly look at another way to define a derivative. 
...
\begin{tikzpicture}
		...
		\addplot [color = Firebrick2, samples = 100] {x^2 + 200};
		%
		\draw [blue] (axis cs: 10, 0) -- (axis cs: 10, 10^2 + 200);
		\node [
			label = {[yshift = 2ex] 180 : {$t$}	},
			mark = none, 
			fill,
			inner sep = 2pt
		]
		at (axis cs: 10, 0) {};
		%
		\draw [blue] (axis cs: 15, 0) -- (axis cs: 15, 15^2 + 200);
		\node [
			label = {[yshift = 2ex] 180 : {$x$}	},
			mark = none, 
			fill,
			inner sep = 2pt
		]
...
\begin{flalign*}
	...
	\intertext{as required. But, consider also} \Deriv{(6t^2 + 1)}{t}, \ \ \Deriv{(6t^2 + 15)}{t} \, , & \text{ and } \ \Deriv{(6t^2 - 50)}{t} \, .
