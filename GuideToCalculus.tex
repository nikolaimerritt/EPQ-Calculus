...
\begin{flalign}
	...
	\intertext{Prove that these identities also hold when $f(x) = 0$ for both, and $g(x) = 0$ for (2.15)}
	\intertext{Hence find the derivatives of the following functions: }
	& (3x^3 - 5x)^9 \, (2x+5)^7 \nn
	& \frac{1 - 2x^2}{4x^3 - 2x} \nn 
	& \left[ \frac{(3x + 1)^4 \, (12x - 3)}{4x^3} \right]^5 \nn
	\intertext{Prove that}
	x^n &= e^{n \, \ln x} \ \text{ for all real $n$, real $x \ne 0$} \fs
	\intertext{Hence (finally!) prove that}
	\Deriv{(x^n)}{x} &= n \, x^{n - 1} \ \text{ for all real $n$, $x$} \fs \nn
	\intertext{Prove the following miscellaneous identities:}	
	\Deriv{(k^x)}{x} &= k^x \ln k , \ k \ne 0 \nn
	\NthDeriv{(k^x)}{x}{n} &= k^x \ln^n k , \ k \ne 0 \nn
